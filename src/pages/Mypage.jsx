...
function Mypage() {
  ...
  const { memberPosts, isLoading, error } = useSelector((state) => state.members)
  ...
  return (
    <Wrapper>
      ...
      <WinWrapper>
        ...
        <Category />
        <CardsWrapper>
          {cates == 'notdone' ? memberPosts.notdone?.map((item) => <Card key={item.id} item={item} />) : null}
          {cates
            ? memberPosts.notdone?.map((item) => (item.category == cates ? <Card key={item.id} item={item} /> : null))
            : null}
          {cates == 'done' ? memberPosts.done?.map((item) => <Card key={item.id} item={item} />) : null}
        </CardsWrapper>
      </WinWrapper>
    </Wrapper>
  )
